...
def question_details(request, slug):
    ...
    answer_list = Answer.objects.filter(questions=question)
    #adding answer
...
def update_answer(request, id):
    ...
    if form.is_valid():
        form.save()
        return redirect('qdetails', slug=answer.questions.slug)
...
def list_info(request):
    question = Question.objects.filter(author=request.user).order_by('-created_at')
    answer = Answer.objects.filter(author=request.user).order_by('-created_at')
    return render(request, 'list_info.html', {'question':question, 'answer':answer})